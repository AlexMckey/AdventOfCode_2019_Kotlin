...
private infix fun Int.rol(dist: Int) = shl(dist) or ushr(-dist)
...
class HalfSipHash(val k0: Int = _seed1.toInt(), val k1: Int = _seed1.shr(32).toInt()) {
    ...
    private fun ByteArray.getOrFF(i: Int) = if (i < size) get(i).toInt() and 0xff else 0xff
            ...
            by lazy {
                object : AbstractMutableSet<MutableMap.MutableEntry<K, V>>() {
                    ...
                    override fun add(element: MutableMap.MutableEntry<K, V>): Boolean =
                        _del.put(wrap(element.key), element.value) != element.value
